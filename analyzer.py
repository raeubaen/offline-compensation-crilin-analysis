...
def plotSingleResolution(tree,name,title,selection=[],verbose=False,nbins=200,just_gauss=False,time=False):
    ...
    tree.Draw("(30*pred_energy[0] + VD_energy[0] * (1 + rng*0.25/sqrt(VD_energy[0]) ))/(PrimaryEnergy[0]) - 1>>resotemp(1000, 0, 1)", "", "goff")

    resotemp = ROOT.gDirectory.Get("resotemp")
    m = resotemp.GetMean()
    s = resotemp.GetRMS()
    reso = ROOT.TH1F("reso","resolution",nbins,m-5*s,m+5*s)
    tree.Draw("(30*pred_energy[0] + VD_energy[0] * (1 + rng*0.25/sqrt(VD_energy[0]) ))/(PrimaryEnergy[0]) - 1>> reso",sel, "goff")
